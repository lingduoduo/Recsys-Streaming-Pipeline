...
public class HasMediaCandidateHydrator implements CandidateHydrator {
    private final Map<String, MovieProfile> catalog;

    public HasMediaCandidateHydrator(Map<String, MovieProfile> catalog) {
    ...
    public List<MovieCandidate> hydrate(ScoredMoviesQuery query, List<MovieCandidate> candidates) {
        return candidates.stream()
            .map(candidate -> {
                ...
                return profile == null ? candidate : candidate.withHasMedia(profile.getHasMedia());
            })
            .toList();
    }
}
