...
class RawArchiveSink(validRoot: String, deadLetterRoot: String, val queryIdentity: String) {
  ...
  private def dataManifest(base: String, rowCount: Long, files: Seq[String]): String =
    base + s"row_count=$rowCount\n" + files.map(path => s"file=$path\n").mkString
  ...
  private def parquetInventory(
    ...
    while (files.hasNext) {
      val path = fileSystem.makeQualified(files.next().getPath)
      if (path.getName.endsWith(".parquet"))
        found += path.toString.stripPrefix(qualifiedRoot)
    }
    found.sorted
  }
  ...
  private def validateDataCommitted(
    ...
    val inventory = lines.filter(_.startsWith("file=")).map(_.stripPrefix("file="))
    ...
    if (rowCount < 0L || inventory != physicalInventory || actual != canonical ||
