...
public class LanguageCodeCandidateHydrator implements CandidateHydrator {
    private final Map<String, MovieProfile> catalog;

    public LanguageCodeCandidateHydrator(Map<String, MovieProfile> catalog) {
    ...
    public List<MovieCandidate> hydrate(ScoredMoviesQuery query, List<MovieCandidate> candidates) {
        return candidates.stream()
            .map(candidate -> {
                ...
                return profile == null ? candidate : candidate.withLanguageCode(profile.getLanguageCode());
            })
            .toList();
    }
}
