...
| `RETRIEVAL_BASE` | `/api/v1/retrieval` | `run-movie-category-sim.sh` | Path prefix appended to `SERVICE_URL` for the sim's three retrieval calls. Override it alone when the backend re-mounts `RetrievalRecommendationController` under a different prefix; `SERVICE_URL` stays the origin. The sim names this variable when `/health/live` answers but the prefix does not |
...
| `SPARK_SQL_SHUFFLE_PARTITIONS` | derived from `SPARK_MASTER` | every Spark job | Overrides the shuffle partition count outright. Without it, a `local[*]` master keeps the job's own default and any other master gets 200 — the flat default of 8 is right for a laptop and funnels every cluster shuffle through too few partitions |
| `SPARK_EVENT_LOG_ENABLED` | `false` | every Spark job | Write Spark event logs so a completed run can be inspected after the fact. Off by default; a long-running stream's logs grow without bound |
| `SPARK_EVENT_LOG_DIR` | `/tmp/spark-events` | every Spark job | Where those event logs go. Created if missing; if it cannot be created, event logging stays off rather than failing the job |
