...
**Every removed term already contributes exactly zero** — but the two sites are inert for
*different* reasons, and the distinction matters because one depends on a strictly stronger
condition:

- `blendOfflineScore` adds `deepLearningWeight * clamp(deepLearningScore)` to the numerator and
  `deepLearningWeight` to `weightSum`. Both addends are `0` **because the weight is `0.0`**, whatever
  the score happens to be. This site is inert by configuration.
- `MovieLensOutcomeScorer` is **not** gated by `deepLearningWeight` at all. It applies a hardcoded
  `EXPLOITATION_DL_WEIGHT = 0.15` to `input.dlScore()` directly, plus `0.75 * clamp(dlScore)` in the
  base logit. These are zero **because `dlScore` itself is always `0.0` at runtime** — the
  `deepLearningEnabled` gate in `HybridRecommendationService:212` prevents `predictBatch` from ever
  running, so `dlScores.getOrDefault(item, 0.0)` returns `0.0` for every candidate. This site is
  inert by data flow, not by weight.

The practical consequence: the scorer's removal is behavior-preserving only while that gate holds.
Were `deepLearningWeight` ever set above zero, the scorer would already be producing different
scores today — so the condition being relied on is the same one the repository already relies on,
but it must be stated as a property of the *value*, not the weight.
...
| Test | Why it changes |
|---|---|
| `RecommendationConstantsTest.deepLearningScoreIsClampedIntoRange` | entirely about dlScore clamping; deleted |
| `RecommendationConstantsTest.offlineScoreStaysInRangeWhenWeightsSumAboveOne` | passes a `0.15` dl weight to make the weights sum to 1.15; rewritten against the surviving three weights |
