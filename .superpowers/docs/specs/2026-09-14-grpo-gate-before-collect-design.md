...
3. The gate predicate appears in the executed plan of the query `toGroups` collects from. Asserted with a `QueryExecutionListener` on a fixture of 5 keepable and 95 zero-variance slates, checking the plan contains `isnull(grpo_drop_reason)` and an `InMemoryTableScan`. Counting rows is NOT sufficient and was tried first: every value `toGroups` returns is identical whether the gate runs in Spark or on the driver after collecting everything, so a count assertion on a separately built frame proves nothing about the real query. A mutant that collected all 100 rows and filtered on the driver -- exactly the behavior this design removes -- passed the entire suite under that earlier assertion. The plan assertion kills it, and also kills removing the `persist`, which is otherwise undetectable and doubles the gate and parse work.
...
Two behaviors genuinely improve rather than being preserved, which is why the acceptance list treats them separately. A slate arriving with no `items` array at all used to crash the micro-batch with a `NullPointerException` from calling `.size` on the null `getSeq` returned; it is now gated as `TooSmall` like any other slate too small to form a group. This was found while reviewing the change rather than designed in, and it is an improvement — a single malformed slate should not fail a batch — but it means the "identical for every input" constraint above holds for every input the old path survived, not literally every input.

The second is a null `item_features` map. Master's `classify` called `getOrElse` straight on it and threw `NullPointerException`; a single `featuresOf` helper now returns an empty map, so the slate fails the feature-version gate like any other unusable vector. This one is only reachable today because "no `grpo_x`" and "no features map" always coincide — a slate with a null map is dropped as `badFeatureVersion` before the group builder ever reads it — but the safety was coincidental rather than structural, and routing both readers through one helper makes it structural.

A cost the earlier draft left unnamed: this adds a `MEMORY_AND_DISK` persist of the full `items` payload on every micro-batch inside `foreachBatch`, so a ten-second trigger means block-manager churn and possible disk spill at that cadence. `unpersist()` is non-blocking, which is harmless here because each batch builds a fresh relation, but it does leave a brief window of lingering blocks. The persist is load-bearing rather than defensive: the collected path contains no `Exchange`, so nothing would be reused between the two actions and dropping it exactly doubles the `from_json` and gate work — measured as 50 gate evaluations with it against 100 without, on a 50-row frame.
