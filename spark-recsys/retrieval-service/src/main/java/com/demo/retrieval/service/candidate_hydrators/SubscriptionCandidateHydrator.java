...
public class SubscriptionCandidateHydrator implements CandidateHydrator {
    private final Map<String, MovieProfile> catalog;

    public SubscriptionCandidateHydrator(Map<String, MovieProfile> catalog) {
    ...
    public List<MovieCandidate> hydrate(ScoredMoviesQuery query, List<MovieCandidate> candidates) {
        return candidates.stream()
            .map(candidate -> {
                ...
                return profile == null ? candidate : candidate.withSubscriptionAuthorId(profile.getSubscriptionAuthorId());
            })
            .toList();
    }
}
